...
export function initDonorPortal(): void {
  ...
  function renderAuthForm(): void {
    ...
    card!.innerHTML = `
      <h3 style="font-family:var(--serif);font-size:1.2rem;color:var(--dark);margin-bottom:8px">Donor Log In</h3>
      <p class="donor-hint" style="margin:0 0 16px;text-align:left">Log in with the account details our team set up for you.</p>
      <form class="donor-form" id="donorForm">
        ...
        ${errorMsg ? `<p class="donor-error">${escapeHtml(errorMsg)}</p>` : ''}
        <button type="submit" class="submit-btn" ${busy ? 'disabled' : ''}>
          <i class="fas fa-user-lock"></i> ${busy ? 'Please wait…' : 'Log In'}
        </button>
        <p class="donor-hint">Don't have login details yet? Contact our team to get set up.</p>
        <p class="donor-hint"><a href="#" id="forgotLink">Forgot password?</a></p>
      </form>
    `;
