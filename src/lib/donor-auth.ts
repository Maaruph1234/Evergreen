...
export async function fetchOutreachRecords(): Promise<OutreachRecord[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, outreach_no, entry_date, surname, other_names, date_of_birth, sex, age, phone, occupation, address, next_of_kin, next_of_kin_address, created_at'
    )
    .order('outreach_no', { ascending: false })
    .order('created_at', { ascending: true });
  if (error) throw error;
  return (data ?? []) as OutreachRecord[];
}

export async function addOutreachRecord(entry: NewOutreachRecord): Promise<void> {
  if (!supabase) throw new Error('Not configured.');
  const { error } = await supabase.from('outreach_records').insert(entry);
  if (error) throw error;
}

export async function updateOutreachRecord(id: string, entry: Partial<NewOutreachRecord>): Promise<void> {
  if (!supabase) throw new Error('Not configured.');
  const { error } = await supabase.from('outreach_records').update(entry).eq('id', id);
  if (error) throw error;
}
