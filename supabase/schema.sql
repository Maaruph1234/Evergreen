...
-- 7. Outreach attendance register: staff log attendees during field outreach
--    visits (health camps, cohort sessions, etc). Any admin can read, add,
--    edit, or remove entries. Grouped by outreach_no for reporting/export.
create table if not exists public.outreach_records (
  id uuid primary key default gen_random_uuid(),
  outreach_no text not null,
  entry_date date not null default current_date,
...
create policy "outreach: admin reads"
  on public.outreach_records for select
  using (public.is_admin());

create policy "outreach: admin inserts"
  on public.outreach_records for insert
  with check (public.is_admin());

create policy "outreach: admin updates"
  on public.outreach_records for update
  using (public.is_admin());

create policy "outreach: admin deletes"
